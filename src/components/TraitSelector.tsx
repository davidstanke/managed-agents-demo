...
export const TraitSelector: React.FC<TraitSelectorProps> = ({ profile, onChangeProfile }) => {
  ...
  return (
    <div className="space-y-6">
      {/* 1. COAT APPEARANCE */}
      <section className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm p-5 sm:p-6 rounded-3xl border border-amber-200/90 dark:border-slate-800 shadow-sm transition-colors">
        <div className="flex items-center justify-between mb-3.5">
          ...
          {profile.coat && (
            <button
              ...
              className="text-[11px] font-semibold text-orange-600 dark:text-orange-400 dark:hover:text-orange-300 hover:underline"
            >
              Clear
            </button>
          )}
        ...
        <div className="flex items-center justify-between mb-3.5">
          ...
          {profile.personalities.length > 0 && (
            <button
              ...
              className="text-[11px] font-semibold text-amber-700 dark:text-amber-400 dark:hover:text-amber-300 hover:underline"
            >
              Clear
            </button>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <section className="lg:col-span-2 bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm p-5 sm:p-6 rounded-3xl border border-amber-200/90 dark:border-slate-800 shadow-sm transition-colors">
          <div className="flex items-center justify-between mb-3.5">
            <div className="flex items-center gap-2">
            ...
            {profile.themes.length > 0 && (
              <button
                ...
                className="text-[11px] font-semibold text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 hover:underline"
              >
                Clear
              </button>
            )}
            ...
            {THEME_OPTIONS.map((theme) => {
              ...
              return (
                <button
                  ...
                  className={`group flex items-start gap-2.5 p-3 rounded-2xl border text-left transition-all duration-200 active:scale-95 ${
                    isSelected
                      ? 'border-purple-500 dark:border-purple-500 bg-purple-50 dark:bg-purple-950/40 ring-2 ring-purple-400/40 dark:ring-purple-400/40 shadow-sm'
                      : 'border-slate-200 dark:border-slate-800 hover:border-purple-200 dark:hover:border-purple-800/50 bg-white dark:bg-slate-800/60 hover:bg-purple-50/30 dark:hover:bg-slate-800'
                  }`}
                >
                  <span className="text-xl shrink-0 mt-0.5 transform group-hover:scale-110 transition-transform">
                    {theme.emoji}
        ...
        {/* Gender Preference */}
        <section className="bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm p-5 sm:p-6 rounded-3xl border border-amber-200/90 dark:border-slate-800 shadow-sm flex flex-col justify-between transition-colors">
          <div>
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 p-3 rounded-2xl border font-semibold text-xs transition-all duration-200 active:scale-95 ${
                      isSelected
                        ...
                        : 'border-slate-200 dark:border-slate-800 hover:border-rose-200 dark:hover:border-rose-800/50 bg-white dark:bg-slate-800/60 text-slate-700 dark:text-slate-300'
                    }`}
                  >
                    <span>{g.icon}</span>
                    <span>{g.label}</span>
