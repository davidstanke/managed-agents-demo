...
export const NameCard: React.FC<NameCardProps> = ({
  ...
  const rankStyle = rankColors[rank] || {
    badge: 'bg-slate-700 dark:bg-slate-800 text-white dark:text-slate-200',
  ...
  return (
    <div
      className={`relative group bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm rounded-3xl p-5 sm:p-6 border ${rankStyle.border} shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between animate-popIn`}
    ...
    >
      <div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`p-2 rounded-xl border transition-all active:scale-90 flex items-center justify-center ${
                copied
                  ? 'bg-emerald-50 dark:bg-emerald-950/50 border-emerald-300 dark:border-emerald-700/60 text-emerald-600 dark:text-emerald-400'
                  : 'bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-700 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100'
              }`}
            ...
            <button
              ...
              className={`p-2 rounded-xl border transition-all active:scale-90 flex items-center justify-center ${
                isFavorite
                  ? 'bg-rose-50 dark:bg-rose-950/50 border-rose-300 dark:border-rose-700/60 text-rose-500 dark:text-rose-400 shadow-sm shadow-rose-200 dark:shadow-rose-950/50'
                  : 'bg-slate-50 dark:bg-slate-800/80 hover:bg-rose-50 dark:hover:bg-rose-950/40 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 hover:border-rose-200 dark:hover:border-rose-800/50'
              }`}
        ...
        <div className="mt-3.5 p-3 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200/50 dark:border-amber-800/40">
          ...
          {nameEntry.vibe && (
            <div className="mt-2 text-[11px] text-amber-900/80 dark:text-amber-300/80 italic flex items-center gap-1.5">
