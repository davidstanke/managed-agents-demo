"""Port discovery and management for the local reviewer agent server."""
...
def get_cache_file() -> Path:
    ...
    return scratch_dir / "reviewer_port.txt"
...
async def is_reviewer_active(port: int, host: str = DEFAULT_HOST) -> bool:
    """Checks if a reviewer server is responding on the given port."""
    ...
    try:
        async with httpx.AsyncClient(timeout=1.0) as client:
            ...
            if resp.status_code == 200:
                data = resp.json()
                return data.get("agent") == "reviewer_agent" or data.get("status") == "ok"
    except Exception:
        pass
    return False
...
async def find_active_server(
    ...
    """Scans for a currently running reviewer agent server."""
    cached = read_cached_port()
    if cached and await is_reviewer_active(cached, host):
        return cached
