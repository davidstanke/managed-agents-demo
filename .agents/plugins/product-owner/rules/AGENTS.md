...
4. **6-Step Specification Lifecycle**:
   ...
   - **Step 2 (Branch Checkout)**: `spec-dra` checks out a new Git branch named `feature/<feature-slug>` (e.g., `feature/user-auth`) matching the feature. If working tree changes exist, they are stashed before checkout and restored afterward.
   - **Step 3 (Write Spec to Disk)**: `spec-dra` writes the initial specification to `docs/specs/<feature_dir>/SPECIFICATION.md` including initial user choices.
